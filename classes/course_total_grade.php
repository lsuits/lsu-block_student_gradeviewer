...
namespace block_student_gradeviewer;
...
class course_total_grade extends \grade_report_overview {
    ...
    public function get_course_total($studentcoursesonly = true) {
        ...
        $decimals = $coursegrade->grade_item->get_decimals();
        $percentage = round(grade_format_gradevalue($finalgrade,
                $coursegrade->grade_item,
                true, GRADE_DISPLAY_TYPE_PERCENTAGE, $decimals), $decimals);

        $value = grade_format_gradevalue($finalgrade, $coursegrade->grade_item, true, GRADE_DISPLAY_TYPE_PERCENTAGE, $decimals);
